        ...
        <div
            ...
            @if (!is_null($error))
                <div class="dz-bg-red-50 dz-p-4 dz-w-full dz-mb-4 dz-rounded dark:dz-bg-red-600">
                    <div class="dz-flex dz-gap-3 dz-items-start">
                        ...
                        <h3 class="dz-text-sm dz-text-red-800 dz-font-medium dark:dz-text-red-100">{{ $error }}
                        </h3>
                    </div>
                </div>
            @endif
            ...
            <div class="dz-flex dz-items-center dz-gap-2 dz-text-gray-500 dz-text-sm dz-mt-2">
                @php
                    ...
                    $hasMinFileSize = !is_null($this->minFileSize);
                    $hasMimes = !empty($this->mimes);
                @endphp

                @if ($hasMaxFileSize)
                    <p>{{ __('Up to :size', ['size' => Number::fileSize($this->maxFileSize)]) }}</p>
                @endif
                @if ($hasMinFileSize)
                    <p>{{ __('At least :size', ['size' => Number::fileSize($this->minFileSize)]) }}</p>
                @endif

                @if (($hasMinFileSize || $hasMaxFileSize) && $hasMimes)
            ...
            @if (count($files) > 0)
                <div class="dz-flex dz-flex-wrap dz-gap-x-10 dz-gap-y-2 dz-justify-start dz-w-full dz-mt-5"
                    ...
                    @php
                        ...
                        if ($hasMinFileSize || $hasMaxFileSize) {
                            $files = collect($files);
                            if ($hasMinFileSize) {
                                $files = $files->sortBy('size')->all();
                            }
                            if ($hasMaxFileSize) {
                                $files = $files->sortByDesc('size')->all();
                            }
                        }
                    @endphp
                    @foreach ($files as $file)
                        @php
                            if (($hasMaxFileSize && $file['size'] > $this->maxFileSize) || ($hasMinFileSize && $file['size'] < $this->minFileSize)) {
                                $fileSizeOkay = false;
                            } else {
                                $fileSizeOkay = true;
                            }
                        @endphp
                        <div
                            class="dz-flex dz-items-center dz-justify-between dz-gap-2 dz-border dz-rounded {{ $fileSizeOkay ? 'dz-border-gray-200' : 'border-danger' }} dz-w-full dz-h-auto dz-overflow-hidden dark:dz-border-gray-700">
                            <div class="dz-flex dz-items-center dz-gap-3">
                                @if ($this->isImageMime($file['extension']))
                                    <div class="dz-flex-none dz-w-14 dz-h-14">
                                        <img src="{{ $file['temporaryUrl'] }}"
                                            class="dz-object-fill dz-w-full dz-h-full" alt="{{ $file['name'] }}">
                                    </div>
                                @else
                                    <div
                                        ...
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                            ...
                                            class="dz-w-8 dz-h-8 dz-text-gray-500">
