<div wire:click.prefetch="toggle('{{ $index }}')"
    ...
    <svg class="absolute text-blue-100 fill-current w-full inset-x-0 bottom-0" viewBox="0 0 314.16 207.25">
...
<div class="@if ($column['hidden']) hidden @else relative h-12 overflow-hidden align-top flex table-cell @endif"
    ...
    @if ($column['sortable'])
        <button wire:click="sort({{ $index }})"
            ...
            <span
                class="inline flex-grow @if ($column['headerAlign'] === 'right') text-right @elseif($column['headerAlign'] === 'center') text-center @endif"">{{ str_replace('_', ' ', $column['label']) }}</span>
