            ...
            <div class="flex flex-wrap items-center space-x-1">
                ...
                @if (count($this->massActionsOptions))
                    <div class="flex items-center justify-center space-x-1">
                        ...
                        <select wire:model="massActionOption"
                            class="px-3 py-1 text-xs font-medium tracking-wider uppercase bg-white border border-green-400 space-x-2 rounded-md leading-4 focus:outline-none"
                        ...
                        <button wire:click="massActionOptionHandler"
                            class="flex items-center px-4 py-1 text-xs font-medium tracking-wider text-green-500 uppercase bg-white border border-green-400 rounded-md leading-4 hover:bg-green-200 focus:outline-none"
                            style="margin-right:10px" type="submit" title="Submit">Go</button>
                    </div>
                @endif
                ...
                @foreach ($columnGroups as $name => $group)
                    <button wire:click="toggleGroup('{{ $name }}')"
                        class="px-3 py-1 text-xs font-medium tracking-wider text-green-500 uppercase bg-white border border-green-400 rounded-md leading-4 hover:bg-green-200 focus:outline-none">
                        <span
                ...
                @foreach ($this->columns as $index => $column)
                    @if ($column['hideable'])
                        <button wire:click.prefetch="toggle('{{ $index }}')"
                            class="px-3 py-1 rounded text-white text-xs focus:outline-none
                            ...
                            @foreach ($this->columns as $index => $column)
                                ...
                                @elseif($column['type'] === 'checkbox')
                                    @unless ($column['hidden'])
                                        <div
                                            class="flex justify-center table-cell w-32 h-12 px-6 py-4 overflow-hidden text-xs font-medium tracking-wider text-left text-gray-500 uppercase align-top border-b border-gray-200 bg-gray-150 leading-4 focus:outline-none">
                                            <div
                                                class="px-3 py-1 rounded @if (count($selected)) bg-orange-400 @else bg-gray-400 @endif text-white text-center">
                                                {{ count($selected) }}
                                            </div>
                                        </div>
                                    @endunless
                    ...
                    @foreach ($this->results as $row)
                        <div class="table-row p-1 {{ $this->rowClasses($row, $loop) }}">
                            @foreach ($this->columns as $column)
                                ...
                                @else
                                    <div class="table-cell  @unless ($column['wrappable']) whitespace-nowrap truncate @endunless @if ($column['contentAlign'] === 'right') text-right @elseif($column['contentAlign'] === 'center') text-center @else text-left @endif {{ $this->cellClasses($row, $column) }}"
                                        ondblclick="window.location.href= '{{ route('order.show', $row->org_order_id) . '?' . session()->get('navigtionQuery') }}'"
                        ...
                        <div class="table-row p-1">
                            @foreach ($this->columns as $column)
                                @unless ($column['hidden'])
                                    @if ($column['summary'])
                                        <div
                                            class="table-cell px-6 py-1 @unless ($column['wrappable']) whitespace-nowrap truncate @endunless @if ($column['contentAlign'] === 'right') text-right @elseif($column['contentAlign'] === 'center') text-center @else text-left @endif {{ $this->cellClasses($row, $column) }}">
                ...
                <div class="items-center justify-between p-2 sm:flex">
                    ...
                    @if (count($this->results))
                        <div class="flex items-center my-1 sm:my-0">
                            <select name="perPage"
                                class="block w-full py-1 pl-3 pr-10 mt-1 text-base border-gray-300 form-select leading-6 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 sm:text-sm sm:leading-5"
                                wire:model="perPage">
