...
class LivewireDatatable extends Component
{
  ...
  public function applyToTable($options)
  {
    ...
    foreach (['perPage', 'search', 'activeSelectFilters', 'activeDateFilters', 'activeTimeFilters', 'activeBooleanFilters', 'activeTextFilters', 'activeNumberFilters', 'hide', 'selected', 'pinnedRecords',] as $property) {
  ...
  public function mount(
    ...
    foreach (['model', 'include', 'exclude', 'hide', 'dates', 'times', 'searchable', 'sort', 'hideHeader', 'hidePagination', 'exportable', 'hideable', 'beforeTableSlot', 'buttonsSlot', 'afterTableSlot',] as $property) {
  ...
  #[Computed]
  public function processedColumns()
  {
  ...
  public function doDateFilterStart($index, $start)
  {
    ...
    if (isset($this->activeDateFilters[$index]['start']) && $this->activeDateFilters[$index]['start'] == '') {
  ...
  public function addDateRangeFilter()
  {
    ...
    $this->query->where(function ($query) {
      foreach ($this->activeDateFilters as $index => $filter) {
        ...
        $dates = [
          isset ($filter['start']) && $filter['start'] != '' ? $filter['start'] : config('livewire-datatables.default_time_start', '1970-01-01'),
          isset ($filter['end']) && $filter['end'] != '' ? $filter['end'] : config('livewire-datatables.default_time_end', '2040-12-31'),
        ];
        $query->whereDate($this->getColumnFilterStatement($index)[0], '>=', $dates[0])
          ->whereDate($this->getColumnFilterStatement($index)[0], '<=', $dates[1]);
      }
    });
  ...
  public function saveUserPrefrences()
  {
    $data = session()->only(['orders_perpage', 'orders_search', 'orders_hidden_columns', 'orders_filter']);
    auth()->user()->update(['settings' => $data]);
  }

  // update value in filter according to new filters
  ...
  public function updateFilterValues()
  {
    ...
    $filterQuery = $this->query->toBase();

    foreach ($this->columns as $index => $col) {
      if (is_array($col['filterable'])) {
        $filterQuery = clone $this->query;
        $col_name = Str::startsWith($col['name'], 'callback_') ? $this->getColumnFilterStatement($index)[0] : $col['name'];
        if (!$this->freshColumns[$index]['extraCollection'])
          $this->columns[$index]['filterable'] = $filterQuery->groupBy($col_name)->pluck($col_name)->toArray();
        else {
          $this->columns[$index]['filterable'] = $this->freshColumns[$index]['extraCollection']($filterQuery);
        }
      }
    }
  }
  ...
  {
    return $this->query->pluck('checkbox_attribute');
  }
